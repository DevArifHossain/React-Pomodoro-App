  ...
  onTimerStart = () => {
    const calcTime = () => {
      ...
      if (sec > 0) {
        ...
        if (sec == 0 && min == 0) {
          // trying to NOT get any negative time
  ...
  render() {
    return (
      <div id="timer-con">
        ...
        <div className="btns">
          <button className="btn start" onClick={this.onTimerStart} disabled={this.state.min !== 25}>
            Start
          </button>
          <button className="btn reset" onClick={this.onTimerReset}>
            Reset
          </button>
        </div>
      </div>
